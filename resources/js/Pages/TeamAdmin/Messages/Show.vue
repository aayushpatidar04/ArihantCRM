...
const customerPhone = computed(() => {
    return props.customer.phone || "No phone number";
});
...
const loadOlderMessages = async () => {
    ...
    try {
        const response = await axios.get(
            route("team-admin.messages.history", {
...
const jumpToDate = async () => {
    ...
    try {
        ...
        const targetMessages = (data.messages || []).map(normalizeMessage);

        messageList.value = targetMessages;
...
const bodyComponent = computed(() => {
    return getTemplateComponent("BODY");
});

const headerComponent = computed(() => {
    return getTemplateComponent("HEADER");
});

const buttonsComponent = computed(() => {
    return getTemplateComponent("BUTTONS");
});
...
const getBodyVariableCount = () => {
    return getVariableCount(getBodyText());
};

const getHeaderVariableCount = () => {
    if (String(headerComponent.value?.format || "").toUpperCase() !== "TEXT") {
        return 0;
    }

    return getVariableCount(headerComponent.value?.text || "");
};
...
const hasTemplateHeader = computed(() => {
    return Boolean(headerComponent.value);
});

const hasTemplateHeaderVariables = computed(() => {
    return templateHeaderVariables.value.length > 0;
});

const hasTemplateBodyVariables = computed(() => {
    return templateBodyVariables.value.length > 0;
});

const hasTemplateHeaderMedia = computed(() => {
    return ["IMAGE", "VIDEO", "DOCUMENT"].includes(templateHeaderFormat.value);
});

const hasTemplateButtonVariables = computed(() => {
    return templateButtonVariables.value.length > 0;
});
...
const sendTextMessage = async () => {
    ...
    try {
        const response = await axios.post(
            route("team-admin.messages.send-text", props.customer.id),
            {
                type: "text",
                body,
            },
...
const sendAttachment = async () => {
    ...
    try {
        const response = await axios.post(
            route("team-admin.messages.send-media", props.customer.id),
            form,
            {
                headers: {
...
const sendTemplate = async () => {
    ...
    try {
        const response = await axios.post(
            route("team-admin.messages.send-template", props.customer.id),
            {
...
const markConversationRead = () => {
    router.post(
        route("team-admin.messages.mark-read", props.customer.id),
        {},
        {
            preserveScroll: true,
            preserveState: true,
                                ...
                                <div>
                                    ...
                                    >
                                        <template v-if="windowOpen">
                                            You can send text, images, documents
                                            and other normal messages.
                                        </template>

                                        <template v-else>
                                            A WhatsApp template is required to
                                            start or reopen this conversation.
                                        </template>
                                    </p>
                                </div>
                            </div>
                    ...
                    <div v-if="composerMode === 'template'" class="p-4">
                        ...
                        >
                            ...
                            <p class="text-[11px] text-amber-700 mt-1">
                                ...
                                approved template for this WhatsApp number.
                            </p>
                        </div>
                            ...
                            >
                                ...
                                <div class="flex flex-wrap gap-2 mt-3">
                                    <span
                                        ...
                                        class="text-[10px] px-2 py-1 rounded-md bg-white border border-surface-200 text-surface-600"
                                    >
                                        {{ templateBodyVariables.length }}
                                        body variable{{
                                            templateBodyVariables.length === 1
                                                ? ""
                                                : "s"
                                        }}
                                    </span>
                        ...
                        >
                            The 24-hour messaging window is closed. A template
                            is required to start or reopen the conversation.
                        </p>
                    </div>
                        ...
                        <div class="p-4">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                ...
                                <div class="space-y-3">
                                        ...
                                        >
                                            <div>
                                                ...
                                                >
                                                    Header
                                                    {{
                                                        templateHeaderFormat.toLowerCase()
                                                    }}
                                                </p>
                                    ...
                                    <div
                                        v-if="
                                            !hasTemplateBodyVariables &&
                                            !hasTemplateHeaderVariables &&
                                            !hasTemplateHeaderMedia &&
                                            !hasTemplateButtonVariables
                                        "
                                        class="rounded-xl border border-surface-200 bg-white p-4"
                                    >
                                        <p
                                        ...
                                        >
                                            This template can be sent directly.
                                        </p>
                                    </div>
                                </div>
                                                ...
                                                >
                                                    ...
                                                    <span
                                                        v-if="
                                                            ...
                                                            button.previewUrl
                                                        "
                                                        class="block text-[9px] text-surface-400 mt-0.5 truncate"
                                                    >
                                                        {{ button.previewUrl }}
                                                    </span>
                                                </div>
